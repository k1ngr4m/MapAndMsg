...
public class RouteTask implements OnRouteSearchListener {
	...
	public interface OnRouteCalculateListener {
		public void onRouteCalculate(float cost, float distance, int duration, DriveRouteResult driveRouteResult);

	}

	public static RouteTask getInstance(Context context) throws AMapException {
		if (mRouteTask == null) {
	...
	private RouteTask(Context context) throws AMapException {
		...
		if (resultCode == AMapException.CODE_AMAP_SUCCESS && driveRouteResult != null) {
			synchronized (this) {
				for (OnRouteCalculateListener listener : mListeners) {
					...
					listener.onRouteCalculate(cost, distance, duration,driveRouteResult);
				}
